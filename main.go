...
func main() {
	...
	if len(os.Args) >= 2 {
		if os.Args[1] != "top" && os.Args[1] != "best" && os.Args[1] != "new" {
			log.Fatalf("%s is not a valid type of story", os.Args[1])
		}
		storyType = os.Args[1]
	}
	if len(os.Args) == 3 {
		var err error
		maxItems, err = strconv.Atoi(os.Args[2])
		if err != nil {
			log.Fatalf("argument should be a number got '%s' instead", os.Args[2])
		}
	}
